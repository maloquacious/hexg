...
func NewVerticalEvenQLayout(size, origin Point) VerticalEvenQLayout {
	return VerticalEvenQLayout{
		size:   size,
		origin: origin,
	}
}

// VerticalEvenQLayout returns a layout with vertical layout (flat-top hexes)
// that shoves even columns down.
type VerticalEvenQLayout struct {
	// size and origin are used when calculating screen pixels.
	size   Point
	origin Point
}

func (l VerticalEvenQLayout) DirectionToBearing(direction int) string {
	// we must coerce direction to 0 ... 5
	return verticalDirectionToBearing[(6+(direction%6))%6]
}

func (l VerticalEvenQLayout) HexagonalGrid(center Hex, radius int) GridStore {
	gs := GridStore{}
	N := radius
	for q := -N; q <= N; q++ {
		r1 := max(-N, -q-N)
...
func (l VerticalEvenQLayout) HexCorner(h Hex, corner int) Point {
	center := l.HexToPixel(h)
	offset := l.PolygonCornerOffset(corner)
	return Point{X: center.X + offset.X, Y: center.Y + offset.Y}
}

func (l VerticalEvenQLayout) HexCorners(h Hex) [6]Point {
	center := l.HexToPixel(h)
...
func (l VerticalEvenQLayout) IsHorizontal() bool {
	return false
}

func (l VerticalEvenQLayout) IsVertical() bool {
	return true
}

func (l VerticalEvenQLayout) OffsetColRowToHex(col, row int) Hex {
...
func (l VerticalEvenQLayout) ParallelogramGrid(q1, r1, q2, r2 int) GridStore {
	gs := GridStore{}
	for q := q1; q <= q2; q++ {
		for r := r1; r <= r2; r++ {
...
func (l VerticalEvenQLayout) PixelToFractionalHex(p Point) FractionalHex {
	M := verticalOrientation
...
func (l VerticalEvenQLayout) PixelToHexRounded(p Point) Hex {
	return l.PixelToFractionalHex(p).Round()
}

func (l VerticalEvenQLayout) PolygonCornerOffset(corner int) Point {
	M := verticalOrientation
	size := l.size
...
func (l VerticalEvenQLayout) PolygonCornerOffsets() [6]Point {
	var corners [6]Point
...
func (l VerticalEvenQLayout) RectangularGrid(center Hex, left, right, top, bottom int) GridStore {
	gs := GridStore{}
...
func (l VerticalEvenQLayout) TriagonalGrid(side_length int) GridStore {
	gs := GridStore{}
